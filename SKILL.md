...
### Q1. `join-agent`만 하면 자동으로 계속 움직이나요?
아니요. `join-agent`는 "게스트 등록"만 합니다.
실제 상태 변화/위치 변화는 `agent-push`가 들어와야 반영됩니다.

### Q2. 그럼 A안은 사용자 OpenClaw 봇과 직접 연결된 건가요?
엄밀히 말하면 "직접 세션 연동"은 아닙니다.
A안은 `office-agent-push.py`가 HTTP로 상태를 전달하는 **push 기반 브리지 방식**입니다.

### Q3. `python office-agent-push.py` 한 번 실행하면 뭐가 되나요?
보통 아래를 자동 수행합니다.
1. 최초 1회 `join-agent` (게스트 등록)
2. 이후 주기적으로 `agent-push` (상태 업데이트)

### Q4. 상태는 랜덤으로 바뀌나요?
아니요. 랜덤이 아닙니다.
스크립트는 로컬 상태 소스(`state.json` 또는 `/status`)를 읽고, 규칙 기반으로 상태를 매핑해 push합니다.

### Q5. 왜 게스트가 멈춘 것처럼 보일 때가 있나요?
주기 push가 끊기면 마지막 상태로 멈춰 보일 수 있습니다.
또한 일정 시간 push가 없으면 offline 처리될 수 있으니, 주기 실행을 유지해야 합니다.

### Q6. A안/B안 중 무엇을 먼저 쓰는 게 좋나요?
처음엔 A안(빠른 연결/디버깅 쉬움), 운영 안정화 후 B안(자동 동기화 고도화) 순서를 권장합니다.
